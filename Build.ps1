...
exec { & dotnet test .\Peppermint.Tests -c Release }

echo "============================= BUILD NUGET PACKAGE ==========================="
$tagOfHead = iex 'git tag -l --contains HEAD'
$prefixExpected = $tagOfHead + "-"
$projectJsonVersion = Get-Content '.\Peppermint\project.json' | Out-String | ConvertFrom-Json | select -ExpandProperty version

if ([string]::IsNullOrEmpty($tagOfHead)) {
  $revision = @{ $true = $env:APPVEYOR_BUILD_NUMBER; $false = 1 }[$env:APPVEYOR_BUILD_NUMBER -ne $NULL];
  $revision = "b{0:D5}" -f [convert]::ToInt32($revision, 10)
  exec { & dotnet pack .\Peppermint -c Release -o .\artifacts --version-suffix=$revision }
} elseif ($projectJsonVersion.StartsWith($prefixExpected,"CurrentCultureIgnoreCase")) {
  exec { & dotnet pack .\Peppermint -c Release -o .\artifacts }
} else {
